defmodule MoreWeb.UserAuthTest do
  use MoreWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias More.Accounts
  alias More.Accounts.Scope
  alias MoreWeb.UserAuth

  import More.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MoreWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      MoreWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        MoreWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
